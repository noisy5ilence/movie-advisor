...
import tracker from '../parsers/tracker';
...
export async function GET(request: Request) {
  ...
  // } catch (error) {
  //   return NextResponse.json([{ title: JSON.stringify(error) }]);
  // }

  const torrents = await tracker.search({ query: params.query, page: parseInt(params.page || '0') });
